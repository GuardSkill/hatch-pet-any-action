---
name: hatch-pet-any-action
description: Create, repair, validate, preview, and package Codex-compatible animated pets and pet spritesheets from character art, screenshots, generated images, or visual references. Use when a user wants a custom pet with inferred or explicit action sets, including fewer or more than nine actions, while still packaging compatible pet.json and spritesheet.webp outputs. This skill composes the installed $imagegen system skill for visual generation and uses bundled scripts for deterministic spritesheet assembly.
---

# Hatch Pet Any Action

This variant keeps the same package shape as `hatch-pet` but supports dynamic action lists. The preferred package contract is still `pet.json + spritesheet.webp`, with `pet.json` extended to include `actions`, `atlas`, `cellWidth`, and `cellHeight`. When the user only provides natural-language behavior descriptions, infer the action list before row generation.
...
Hard boundary: do not create, draw, tile, warp, mirror, or synthesize pet visuals with local Python/Pillow scripts, SVG, canvas, HTML/CSS, or other code-native art as a substitute for `$imagegen`. For a normal pet run, expect 1 base job plus one row-strip job per inferred action. The only exception is `running-left`, which may be derived by mirroring `running-right` only after `running-right` has been generated, visually inspected, and explicitly approved as safe to mirror. If mirroring is not appropriate, generate `running-left` as a normal grounded `$imagegen` row. If those calls are too expensive, blocked, or unavailable, stop and explain the blocker instead of fabricating row strips locally.
...
- `Picturing <Pet>'s poses.` Create the pose rows starting with the highest-signal identity checks in the manifest, typically `idle` plus one prominent locomotion or work state when present. Only consider mirroring `running-left` when `running-right` is also present and clearly works when flipped.
...
```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/hatch-pet-any-action"
...
`prepare_pet_run.py` creates one layout guide image per inferred action under `references/layout-guides/`. Row jobs attach the matching guide as a layout-only input so the model can follow the correct frame count, spacing, centering, and safe padding. Treat these guides as invisible construction references: the generated row strip must not include visible boxes, borders, center marks, labels, guide colors, or the guide background.
...
If there is any asymmetric side-specific marking, readable text, non-mirrored logo, handed prop, one-sided accessory, lighting cue, or direction-specific pose that would become wrong when flipped, do not mirror. When `running-right` is present, generate `running-left` with `$imagegen` using its row prompt and all listed grounding images, including `decoded/running-right.png` as a gait reference. When `running-right` is absent from the manifest, treat `running-left` as a normal grounded row job with the base and canonical references only.
...
After the base job has been recorded and `references/canonical-base.png` exists, row-strip visual generation must use subagents unless the user explicitly says not to use subagents for this session.

Invocation of this skill is the user's standing approval for row-strip subagent delegation for the current pet run. Do not ask for an extra "yes, use subagents" confirmation when the user has already asked to hatch or repair a pet with this skill. Before row generation, simply state which row jobs are being delegated.

If the current environment or tool policy still refuses subagent spawning without an explicit user phrase, treat that as an external blocker rather than a missing user decision. Explain that the skill already treats this run as authorized, name the blocking policy, and ask only for the minimum override needed to proceed.
...
4. Parent spawns subagents first for the highest-signal manifest rows, typically `idle` plus one prominent locomotion or work state when present.
5. Parent records those first-pass results returned by subagents.
6. If both `running-right` and `running-left` exist, parent decides whether `running-left` is safe to derive by mirror; otherwise parent treats `running-left` like any other normal grounded row job.
...
No silent sequential fallback: if subagents cannot be used for row-strip visual generation, stop and explain whether the blocker is user preference or external tool policy before continuing sequentially. Only an explicit user instruction such as "do not use subagents" or "run this sequentially" authorizes a normal sequential row-generation path. The final answer must report which row jobs were delegated to subagents and which, if any, were mirrored or repaired by the parent.
...
Here, `--states all` means every row job currently defined in that run's `imagegen-jobs.json`, not a fixed built-in action list. You may also pass a comma-separated subset of row ids from the manifest, such as `idle,coding,sleeping`.
...
- Treat invocation of this skill for a pet run as the user's default approval for that row-strip delegation. Do not ask for a second confirmation unless the runtime's own subagent policy blocks execution.
...
- When both directional rows exist, delegate `running-right` first, then mirror `running-left` only when visual inspection confirms a mirror preserves identity and semantics; otherwise delegate `running-left` as a normal grounded `$imagegen` row.
...
- Final atlas is PNG or WebP, transparent-capable, and its dimensions match the package manifest metadata.
