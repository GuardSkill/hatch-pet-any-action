...
from action_spec import resolve_action_spec
...
def default_output_dir(pet_id: str) -> Path:
    ...
    return Path.cwd() / "output" / "hatch-pet-any-action" / f"{pet_id}-{timestamp}"
...
def create_layout_guides(run_dir: Path, action_spec: dict[str, object]) -> list[dict[str, object]]:
    guide_dir = run_dir / LAYOUT_GUIDE_DIR
    actions = action_spec["actions"]
    cell_width = int(action_spec["cell_width"])
    cell_height = int(action_spec["cell_height"])
    return [
        create_layout_guide(
            guide_dir / f"{action['id']}.png",
            str(action["id"]),
            int(action["frames"]),
            cell_width=cell_width,
            cell_height=cell_height,
...
def row_prompt(args: argparse.Namespace, action: dict[str, object]) -> str:
    state = str(action["id"])
...
def make_jobs(
    ...
    available_states = {str(action["id"]) for action in actions}
    for action in actions:
        state = str(action["id"])
        frames = int(action["frames"])
        depends_on = ["base"]
        ...
        if state == "running-left" and "running-right" in available_states:
...
def load_actions_input(args: argparse.Namespace) -> dict[str, object]:
    action_request: dict[str, object] = {}
    if args.actions_file:
        payload = json.loads(Path(args.actions_file).expanduser().resolve().read_text(encoding="utf-8"))
        if isinstance(payload, dict):
            action_request.update(payload)
    if args.actions_json:
        payload = json.loads(args.actions_json)
        if isinstance(payload, dict):
            action_request.update(payload)
        elif isinstance(payload, list):
            action_request["actions"] = payload
...
def main() -> None:
    ...
    parser.add_argument(
        "--actions-json",
        default="",
        help="Inline JSON array/object describing actions and optional atlas metadata.",
    )
    parser.add_argument(
        "--actions-file",
        default="",
        help="Path to a JSON file describing actions and optional atlas metadata.",
    )
    parser.add_argument(
        "--chroma-key",
        default="auto",
    ...
    action_request = load_actions_input(args)
    ...
    layout_guides = create_layout_guides(run_dir, action_spec)
    actions = action_spec["actions"]
    atlas = {
        **action_spec["atlas"],
        "cell_width": int(action_spec["cell_width"]),
    ...
    for action in actions:
        write_text(
            row_prompt_dir / f"{action['id']}.md",
            row_prompt(args, action),
        )
    ...
    jobs = {
        ...
        "run_dir": str(run_dir),
        "primary_generation_skill": "$imagegen",
        "jobs": make_jobs(run_dir, copied_refs, actions),
    }
