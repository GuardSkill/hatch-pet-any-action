...
- Atlas dimensions match `atlas.columns * cellWidth` by `atlas.rows * cellHeight`.
- Default grid is 8 columns; row count may be fewer or greater than 9.
- Each frame fits inside its declared cell size, default `192x208`.
...
- Each action row uses the exact expected number of frames.
...
1. Single bad frame.
2. One row.
3. Full atlas regeneration only when identity or layout is broadly broken across multiple action rows.
