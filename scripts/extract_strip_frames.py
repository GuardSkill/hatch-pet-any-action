...
from action_spec import load_action_spec_document

CELL_WIDTH = 192
CELL_HEIGHT = 208


def parse_states(raw: str, frame_counts: dict[str, int]) -> list[str]:
    if raw.strip().lower() == "all":
        return list(frame_counts)
    states = [item.strip() for item in raw.split(",") if item.strip()]
    unknown = sorted(set(states) - set(frame_counts))
    if unknown:
...
def main() -> None:
    ...
    action_spec = load_action_spec_document(decoded_dir.parent / "pet_request.json")
    global CELL_WIDTH, CELL_HEIGHT
    CELL_WIDTH = int(action_spec["cell_width"])
    CELL_HEIGHT = int(action_spec["cell_height"])
    frame_counts = {str(action["id"]): int(action["frames"]) for action in action_spec["actions"]}
    states = parse_states(args.states, frame_counts)
    manifest = []
    for state in states:
