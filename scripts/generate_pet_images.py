...
def available_states(manifest: dict[str, object]) -> list[str]:
    return [
        str(job.get("id"))
        for job in manifest_jobs(manifest)
        if str(job.get("id")) and str(job.get("id")) != "base"
    ]


def parse_states(raw: str, *, valid_states: list[str]) -> list[str]:
    if raw.strip().lower() == "all":
        return valid_states
    states = [item.strip() for item in raw.split(",") if item.strip()]
    unknown = sorted(set(states) - set(valid_states))
    if unknown:
...
def main() -> None:
    ...
    valid_states = available_states(manifest)
    jobs = select_jobs(
        manifest,
        states=parse_states(args.states, valid_states=valid_states),
