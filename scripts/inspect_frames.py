...
def load_action_spec(frames_root: Path) -> dict[str, object]:
    manifest_path = frames_root / "frames-manifest.json"
    if not manifest_path.is_file():
        raise SystemExit(f"missing frames manifest: {manifest_path}")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    actions = manifest.get("actions")
    atlas = manifest.get("atlas")
    if not isinstance(actions, list) or not isinstance(atlas, dict):
        raise SystemExit("frames-manifest.json is missing actions or atlas metadata")
    return {
        "actions": actions,
        "atlas": atlas,
...
def main() -> None:
    ...
    action_spec = load_action_spec(frames_root)
